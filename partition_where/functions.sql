...
-- use partition_where_add_part(table_name, 'great', 'value>10') to add partitions
--
-- parameters:
-- 1. name of the table (text)
-- 2. options (hstore)
--      none yet
CREATE OR REPLACE FUNCTION partition_where_init_table(in table_name text, in options hstore default ''::hstore) returns boolean as $$
...
BEGIN
  ...
  insert into partition_where_tables values (table_name, Array[]::text[], Array[]::text[], options);

  -- create trigger for insert statement
  execute 'create or replace function partition_where_insert_trigger_'||table_name||'() returns trigger as $f$ BEGIN perform partition_where_on_insert('''||table_name||''', NEW); return null; END; $f$ language plpgsql;';
  execute 'create trigger partition_where_insert_trigger_'||table_name||' before insert on '||table_name||' for each row execute procedure partition_where_insert_trigger_'||table_name||'();';
  ...
  execute 'create or replace function '||table_name||'_query(in parts text[], in _where text default '''', in options hstore default ''''::hstore) returns setof '||table_name||' as $f$ declare r '||table_name||'%rowtype; sql text; begin sql:=partition_where_compile_query('''||table_name||''', parts, _where, options); return query execute sql; return; end; $f$ language plpgsql;';
  ...
  update partition_where_tables
    set indexes=index_def
    where partition_where_tables.table_name=table_name;
  perform partition_where_table_indexes(table_name, 'other');
...
-- add part
CREATE OR REPLACE FUNCTION partition_where_add_part(in table_name text, in part_id text, in part_where text) returns boolean as $$
#variable_conflict use_variable
DECLARE
BEGIN
  update partition_where_tables set
  ...
  perform partition_where_table_indexes(table_name, part_id);
...
CREATE OR REPLACE FUNCTION partition_where_on_insert(in table_name text, in NEW anyelement) returns boolean as $$
#variable_conflict use_variable
DECLARE
  way geometry;
  table_list int2[];
  ...
  c int;
BEGIN
  select * into table_def from partition_where_tables where partition_where_tables.table_name=table_name;
...
create or replace function partition_where_compile_query(in table_name text, in parts text[], in _where text default '', in options hstore default ''::hstore) returns text as $$
#variable_conflict use_variable
DECLARE
  r record;
  table_def record;
...
create or replace function partition_where_add_index(in table_name text, in index_def text) returns boolean as $$
#variable_conflict use_variable
DECLARE
  table_def record;
  r record;
...
BEGIN
  ...
  for i in array_lower(parts, 1)..array_upper(parts, 1) loop
    perform partition_where_add_index_table(table_name, parts_id[i], index_def);
  end loop;
...
create or replace function partition_where_add_index_table(in table_name text, in part_id text, in index_def text) returns boolean as $$
#variable_conflict use_variable
DECLARE
BEGIN
...
create or replace function partition_where_table_indexes(in table_name text, in part_id text) returns boolean as $$
#variable_conflict use_variable
DECLARE
  table_def record;
  i int;
BEGIN
  ...
  for i in array_lower(table_def.indexes, 1)..array_upper(table_def.indexes, 1) loop
    perform partition_where_add_index_table(table_name, part_id, table_def.indexes[i]);
  end loop;
...
create or replace function partition_where_drop_table(in table_name text) returns boolean as $$
#variable_conflict use_variable
DECLARE
BEGIN
  execute 'drop table '||table_name||' cascade;';
  execute 'delete from partition_where_tables where table_name='||quote_nullable(table_name)||';';
